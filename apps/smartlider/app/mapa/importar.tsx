...
export default function ImportarMapaScreen() {
  ...
  async function selecionarArquivo() {
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: '*/*',                // todos os arquivos — o seletor do Android filtra
        copyToCacheDirectory: true, // copia para cache local (necessário para leitura)
        multiple: false,
      })
  ...
  async function handleCarregar() {
    ...
    try {
      ...
      // 5. Salvar no banco — nome derivado do arquivo, coordenadas zeradas
  ...
  return (
    <View style={st.root}>
      ...
      {/* Área de seleção */}
      <TouchableOpacity
        style={[st.picker, arquivo && st.pickerAtivo]}
        onPress={selecionarArquivo}
        activeOpacity={0.75}
        disabled={uploading}
      >
        <View style={[st.iconWrap, arquivo && st.iconWrapAtivo]}>
          <Ionicons
        ...
        {arquivo ? (
          <>
            ...
            <Text style={st.fileSub}>
              {arquivo.size ? `${(arquivo.size / 1024).toFixed(0)} KB` : ''}
              {arquivo.size ? '  ·  ' : ''}Toque para trocar
            </Text>
          </>
        ) : (
          <>
      ...
      {/* Botão Carregar */}
      <TouchableOpacity
        style={[st.btn, (!arquivo || uploading) && st.btnDisabled]}
